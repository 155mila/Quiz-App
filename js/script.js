...
const showResult = (result, quiz) => {
  ...
  block.innerHTML = `
  <h2 class="main__subtitle main__subtitile_result">Ваш результат</h2>

        <div class="result__box">
          <p class="result__ratio result__ratio_${ratio + 1}">${result}/${
    quiz.list.length
  } </p>
...
const renderQuiz = (quiz) => {
  ...
  const showQuestion = () => {
    const data = quiz.list[countQuestion];
    countQuestion++;
    ...
    form.dataset.count = `${countQuestion}/${quiz.list.length}`;
    ...
    form.addEventListener('submit', (e) => {
      ...
      if (ok) {
        ...
        if (countQuestion < quiz.list.length) {
          showQuestion();
        } else {
